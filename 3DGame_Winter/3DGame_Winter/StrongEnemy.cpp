...
namespace
{
	...
	constexpr float kModelScale = 70.0f; // ƒ‚ƒfƒ‹‚ÌƒXƒP[ƒ‹
	...
	constexpr float kFramesPerSecond = 60.0f; // •b”•ÏŠ·
...
void StrongEnemy::Update()
{
	...
	if (m_invincibilityTimer > 0.0f)
	{
		...
		if (kMaxHp * 0.5f >= m_hp)
		{
			// HP‚ª”¼•ªˆÈ‰º‚È‚ç‹¯‚Ş(ƒ_ƒ[ƒWƒŠƒAƒNƒVƒ‡ƒ“‚ğ‚Æ‚é)
		}
		if (m_invincibilityTimer < 0.0f)
		{
	...
	case StrongEnemy::DEFAULT:
		// ”ÍˆÍUŒ‚‚Ì”»’f UŒ‚”¼Œa‚æ‚è‰“‚­A”ÍˆÍUŒ‚‚ÌÅ‘å”¼Œa‚É‹ß‚¢‹——£‚Ìê‡
		...
		// ’ÊíUŒ‚‚Ì”»’f(ƒLƒƒƒ‰ƒNƒ^[‚ÌÕ“Ë”¼Œa‚Ì‚·‚®ŠO‘¤‚­‚ç‚¢)
...
void StrongEnemy::Draw()
{
	...
	if (m_state == StrongEnemyState::RANGEATTACK_CHARGE)
	{
		...
		float currentRadius = kRangeAttackRadius * progress; // Œ»İ‚Ì”¼Œa‚ğŒvZ(Å‘å”¼Œa*is—¦)
