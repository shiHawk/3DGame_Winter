...
namespace
{
	...
	constexpr float kCloseRangeAttackDistance = 200.0f; // 近距離攻撃のみを行う距離
...
void Companion::Update()
{
	...
	if (m_controlMode == ControlMode::PLAYER)
	{
		...
		if (Pad::isTrigger(PAD_INPUT_2) && !m_isAvoidanceFlag) // 回避
		{
...
void Companion::UpdatePlayerControlState()
{
	...
	case Companion::CompanionState::NORMAL:
		...
		if (m_attackCoolTimer <= 0.0f)
		{
			...
			{
				if (m_distanceToEnemy > 0.0f && m_distanceToEnemy <= kLongRangeAttackDistance) // 敵がいる場合のみ回転
				{
